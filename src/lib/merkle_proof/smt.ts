import { Bool, Encoding, Field } from "snarkyjs"
...
export class SMT {
    ...
    private zeroNode: Field
    // Additional entry value to mark the leaf nodes.
    private entryMark: Field
    ...
    constructor(hash: HashFunction) {
        this.hash = hash
        this.zeroNode = new Field(0);
        this.entryMark = new Field(1);
        this.nodes = new Map()
    ...
    add(key: string, value: Value) {
        ...
        const newNode = this.hash(Encoding.Bijective.Fp.fromString(key).concat([value, this.entryMark]))
        this.nodes.set(newNode, Encoding.Bijective.Fp.fromString(key).concat([value, this.entryMark]))
     ...
     * @returns The root node.
     */
    private addNewNodes(node: Field, path: number[], sidenodes: SideNodes, i = sidenodes.length - 1): Field {
        for (; i >= 0; i--) {
            const childNodes: ChildNodes = path[i] ? [sidenodes[i], node] : [node, sidenodes[i]]
            node = this.hash(childNodes)
     ...
     * @param i The index to start from.
     */
    private deleteOldNodes(node: Field, path: number[], sidenodes: SideNodes) {
        for (let i = sidenodes.length - 1; i >= 0; i--) {
            const childNodes: ChildNodes = path[i] ? [sidenodes[i], node] : [node, sidenodes[i]]
            node = this.hash(childNodes)
...
export type ChildNodes = Field[]
export type SideNodes = Field[]
export type Value = Field[]

export type HashFunction = (childNodes: ChildNodes) => Field
